...
# Enable 32-bit applications in IIS app pools
RUN powershell -Command \
    Import-Module WebAdministration; \
    Get-ChildItem IIS:\AppPools | ForEach-Object { Set-ItemProperty -Path "IIS:\AppPools\$($_.Name)" -Name enable32BitAppOnWin64 -Value $true }

# Run additional configuration script
RUN powershell -ExecutionPolicy Bypass -File /scripts/configuration.ps1
