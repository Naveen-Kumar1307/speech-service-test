...
namespace GlobalEnglish.Recognition.Repository
{
    ...
    public class ConnectionFactory
    {
        ...
        public static string GetConnectionString(string connectionName)
        {
            ...
            {
                return RoleEnvironment.GetConfigurationSettingValue(connectionName);
            }
            else
            {
                return ConfiguredValue.ConnectionNamed(connectionName);
            }
        }
    ...
    {
        private static readonly string AzureDatabaseHostSuffix = "database.windows.net";
        private static readonly ILog Logger = LogManager.GetLogger(typeof(ConnectionExtensions));
        private static RetryManager CachedRetryManager = null;

        /// <summary>
        /// Indicates whether the connected database is hosted in Azure.
        /// </summary>
        /// <param name="connection">this connection</param>
        /// <returns>whether the connected database is hosted in Azure</returns>
        public static bool IsAzureHosted(this SqlConnection connection)
        {
            if (Argument.IsAbsent(connection) || 
                Argument.IsAbsent(connection.DataSource)) return false;

            return connection.DataSource.EndsWith(AzureDatabaseHostSuffix);
        }

        /// <summary>
        /// Reliably fetches data using this connection.
        /// </summary>
        /// <typeparam name="ValueType">a result type</typeparam>
        /// <param name="connection">this connection</param>
        /// <param name="FetchResults">a fetch action</param>
        /// <returns>the result(s)</returns>
        public static ValueType ExecuteReliably<ValueType>(this SqlConnection connection,  Func<ValueType> FetchResults)
        {
            try
            {
                var retryManager = GetRetryManager(connection);
                ...
                else
                {
                    ...
                    try
                    {
                        return retryPolicy.ExecuteAction(FetchResults);
                    }
                    finally
                    {
                        connection.Close();
        ...
        /// <summary>
        /// Reliably commits changes using this connection.
        /// </summary>
        /// <param name="connection">this connection</param>
        /// <param name="CommitChanges">a commit action</param>
        public static void ExecuteReliably(this SqlConnection connection, Action CommitChanges)
        {
            try
            {
                var retryManager = GetRetryManager(connection);
                ...
                else
                {
                    ...
                    try
                    {
                        retryPolicy.ExecuteAction(CommitChanges);
                    }
                    finally
                    {
                        connection.Close();
        ...
        /// <summary>
        /// Returns the configured retry manager (or null).
        /// </summary>
        [MethodImpl(MethodImplOptions.Synchronized)]
        private static RetryManager GetRetryManager(SqlConnection connection)
        {
            if (CachedRetryManager == null && connection.IsAzureHosted())
            {
                CachedRetryManager = EnterpriseLibraryContainer.Current.GetInstance<RetryManager>();
                var retryPolicy = CachedRetryManager.GetDefaultSqlConnectionRetryPolicy();
                retryPolicy.Retrying += new EventHandler<RetryingEventArgs>(ReportConnectionRetry);
            }

            return CachedRetryManager;
        }
