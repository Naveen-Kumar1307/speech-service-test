...
// 
// FLV Extract
// Copyright (C) 2006-2010  J.D. Purcell (moitah@yahoo.com)
// 
...
namespace JDP {
	...
	public class FLVFile : IDisposable {
        ...
        public FLVFile(Stream readStream)
        {
        ...
        public void ExtractStreams(Stream audioStream, Stream videoStream)
        {
            _audioStream = audioStream;
            _videoStream = videoStream;
            ExtractStreams(audioStream != null, videoStream != null, false, null);
        }
		...
		public void ExtractStreams(bool extractAudio, bool extractVideo, bool extractTimeCodes, OverwriteDelegate overwrite) {
			...
			if (_outputDirectory.Length > 0 && !Directory.Exists(_outputDirectory)) {
		...
		private IAudioWriter GetAudioWriter(uint mediaInfo) {
			...
			if ((format == 2) || (format == 14)) { // MP3
                ...
                {
                    return new SpeexWriter(_audioStream, (int)(_fileLength & 0xFFFFFFFF));
                }
                else
                {
	...
	class SpeexWriter : IAudioWriter {
        ...
        public SpeexWriter(Stream outputStream, int serialNumber)
        {
            _path = outputStream.GetType().Name;
